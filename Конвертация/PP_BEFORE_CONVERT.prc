...
begin
  ...
  */
  ...
  /*
  TODO: Обработать подразделения с одним и тем же наименованием в разных папках.
  */
  ...
  update P7_ZANKFAM t
     set t.degrel_rn =
         (select min(t.degrel_rn) from P7_ZDEGREL t where t.code in ('----', 'Прочее'))
  ...
  -- ЗАРАБОТНАЯ ПЛАТА:
  begin
    select t.hrtype_rn into shrtype_rn from P7_ZHRTYPE t where t.nick = 'Д';
  exception
  ...
  update P7_ZFZPFUNC t set t.params = replace(t.params, '/Ст=', '/СТ=');
  update P7_ZFZPFUNC t set t.params = replace(t.params, '/Шк=', '/ШК=');
  ...
  if nTEMP > 0 then
    ...
    exception
      when no_data_found then
        p_exception(0, 'Необходимо добавить квалификационную категорию "нет категории"');
    end;
